...
export async function resolveAdminTenant(
  ...
  if (admin.is_superadmin) {
    const tenants = await queryRows<Tenant>(
      "SELECT id, slug, name, chat_room_id, event_notice_chat_room_id, chat_room_thread_id, event_notice_chat_room_thread_id FROM tenant ORDER BY name ASC",
    );
  ...
  const tenant = await queryFirst<Tenant>(
    "SELECT id, slug, name, chat_room_id, event_notice_chat_room_id, chat_room_thread_id, event_notice_chat_room_thread_id FROM tenant WHERE id = ? LIMIT 1",
    [admin.tenant_id],
  );
