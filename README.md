...
같은 봇이라도 공동체별로 다른 링크를 쓰면, 사용자가 눌렀을 때 해당 공동체 이벤트 목록으로 바로 들어갑니다.
...
공동체별로 다른 URL을 쓰려면 `url`만 해당 테넌트로 바꾸면 됩니다.
...
- **5-3. Web App이 열렸는지 확인**
  - 화면에 **우리 서비스 화면**(공동체 이름, 이벤트 목록 등)이 뜨면 성공이다.  
